# Troubleshooting Notes — Lab 66 WinRM Activity Investigation
...
| Issue | Resolution |
|---|---|
| WinRM configuration query failed | Ran `winrm quickconfig` |
| WinRM service unavailable | Service became Running/Automatic |
| Listener unavailable before configuration | HTTP listener enabled on 5985 |
| Firewall access required | Domain/Private WinRM rule enabled |
| Service command line contained arguments | Separated executable from parameters |
| No `wsmprovhost.exe` evidence | Documented telemetry result |
| No relevant 4688 | Documented process telemetry gap |
| No targeted 4104 | Documented PowerShell telemetry result |
| No 5985/5986 Event ID 3 | Documented network telemetry result |
| PSReadLine command unavailable | Used `Get-History` |

## Final Troubleshooting Lesson

The main lesson from Lab 66 is:

> **Getting WinRM operational is not the same as demonstrating remote activity.**

A sound investigation separates:
...
Only the latter part can establish evidence of actual remote execution or lateral movement.
