# Investigation Notes — Lab 66 WinRM Activity Investigation

## Investigation Overview

The investigation examined the local WinRM configuration and searched for evidence of remote-management activity.

The investigation was intentionally performed without connecting to another Windows host.
...
1. WinRM initially failed to respond to WS-Man configuration requests.
2. `winrm quickconfig` was executed.
3. The WinRM service became Running and Automatic.
4. The service ran as `NT AUTHORITY\NetworkService`.
5. The WinRM process ID was 1672.
6. An HTTP listener was enabled on port 5985.
7. WinRM allowed remote access.
8. WinRM allowed remote shell access.
9. A Domain/Private firewall exception was enabled.
10. The WinRM Operational log was enabled.
11. The log recorded both failed and successful WS-Man operations.
12. No `wsmprovhost.exe` event was identified.
13. No relevant 4688 result was identified.
14. No relevant targeted 4104 result was identified.
15. No 5985/5986 Sysmon Event ID 3 result was identified.
16. No malicious remote-management activity was established.

## Analyst Assessment

The evidence establishes that WinRM was configured and functioning locally after the troubleshooting stage.

The configuration changes themselves generated meaningful forensic evidence in the WinRM Operational log.

However, the investigation did not produce the additional evidence needed to establish remote PowerShell execution or lateral movement.
...
```text
WinRM Configuration Change
+
WinRM Operational Activity
+
Successful Local WS-Man Operations
```

The evidence does not support:

```text
Confirmed Remote PowerShell Session
Confirmed Lateral Movement
Confirmed Malicious WinRM Use
```
